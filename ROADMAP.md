# Roadmap: ресурсная панель (`index-spod-fill.html`)

## Цель

Компактная однофайловая панель ссылок в стиле SPOD fill: навигация слева, фильтры справа, все разделы на одном экране.

## Подзадачи

- [v] Layout SPOD fill: левая навигация, центр, правая панель фильтров
- [v] Сворачиваемые панели — вкладки «Разделы» / «Фильтры» на границе; фильтры скрыты по умолчанию
- [v] Фильтры: поиск, режим, тип, сегмент; «Установить все» / «Снять все» в блоках тип и сегмент
- [v] Градиенты типа (PROM/PSI/IFT) и сегмента (Sigma/Alpha/Universal) на карточках
- [v] Индикатор доступности — кружок слева (серый / зелёный / красный)
- [v] Сегмент S/A/U — буква в цветном кружке справа
- [v] Компактная сетка групп; `adaptGroupsToFit()` — умещение на одном экране
- [v] Заголовок и подсказки перенесены в левую панель; верхняя панель убрана
- [v] `copyCode` скрыт в UI; делегирование копирования
- [v] `index-classic.html` — резерв классического UI; `index.html` → симлинк на SPOD fill
...
- [v] Автодетект кодировок CSV (UTF-8 / windows-1251 / …) + видимый статус загрузки ok/err
...
- [v] 2.4.26: свечной график (single/compare) с подсчётами ТН (ниже/внутри/выше), toggle подписей на графике, heatmap ГОСБ×ТБ
- [v] 2.4.27: мини-toggle подписей зон свечи (↓/↔/↑) отдельно от общего toggle подписей
- [v] 2.4.28: свечи строго по интервалам для всех режимов (в т.ч. разрезы)
- [v] 2.4.29: вид «Свечи·ТН» (границы по суммам, подписи — число ТН у границ); один toggle подписей; улучшенное размещение подписей
- [v] 2.4.30: «Свечи·ТН» — ось Y по числу ТН + укрупнение и усиление читаемости подписей
- [v] 2.4.31: авто-распределение в режиме «Все значения» в сравнении считает по двум периодам (A+B)
- [v] 2.4.32: диагностика авто-распределения (max |Δ| по интервалам) + ограничение K по числу уникальных сумм
- [v] 2.4.33: ошибка загрузки показывает, какие обязательные столбцы не найдены, и список прочитанных заголовков
- [v] 3.0.0: страница `contest-criteria.html` (модули CSS/JS/config) — критерии конкурса; детали в `ROADMAP_CONTEST.md`
