...
Синтетический демо-CSV для sum-distribution.html:
  21_000 строк, 1_900 уникальных ТН (второй период для сравнения), повторы ТН от 1 до 50, разные суммы.
...
OUT = ROOT / "samples" / "sum-distribution-demo-21000.csv"
...
def make_amount(rng: random.Random) -> int:
    """Суммы «другого периода»: чуть сдвинутое распределение."""
    roll = rng.random()
    if roll < 0.06:
        return rng.randint(-4_000_000, -1)
    if roll < 0.10:
        return 0
    # больше умеренных и средних, меньше экстремумов
    if roll < 0.70:
        return rng.randint(1, 3_500_000)
    if roll < 0.92:
        return rng.randint(3_500_001, 12_000_000)
    return rng.randint(12_000_001, 60_000_000)
