...
### 2026-08-10 — Распределение сумм
- Добавлена однофайловая страница `sum-distribution.html` (CSV/вставка, фильтры ТБ/ГОСБ, гистограмма, настройка интервалов)
- Добавлены `config.json`, `ROADMAP.md`, тесты `src/Tests/test_histogram_bins.js`

### Версия 2.3.2 (Текущая)
...
├── src/Tests/                    # Тесты (Playwright)
├── Docs/
│   └── РЕКОМЕНДАЦИИ_ПО_УЛУЧШЕНИЮ.md  # Рекомендации по улучшению кода
└── JS/                           # DevTools-скрипты (редактирование; копируются по кнопкам)
...
└── POST/                         # Копии с расширением .txt (для обмена/архива)
    ├── index.html.txt
    └── JS/*.js.txt
```

**Особенности структуры:**
- Все код основного приложения (HTML, CSS, JavaScript) находится в одном файле `index.html`
- Конфигурация групп и карточек встроена в HTML как JSON-строка
- Документация и рекомендации хранятся в папке `Docs`
- В каталоге `POST` хранятся копии всех файлов кода с добавленным расширением `.txt` (для передачи или архивирования без потери содержимого). Обновление: `python3 tools/sync_post.py`
- DevTools-скрипты редактируются в `JS/`; для встраивания в `index.html` (работа через `file://`): `python3 tools/embed_scripts.py` — **только для разработки**, готовый `index.html` уже содержит встроенные копии и работает без Python
...
Однофайловый инструмент (HTML + CSS + JS) для анализа распределения сумм по интервалам.

**Данные:** столбцы `ТБ`, `ГОСБ`, `сумма`. Загрузка через выбор CSV-файла или вставку таблицы в текстовое окно (`;` / `,` / табуляция; удобно копировать из Excel).

**Возможности:**
- фильтры по ТБ и ГОСБ (мультивыбор);
- разрез: вся выборка / по ТБ / по ГОСБ / по ТБ+ГОСБ;
- интервалы: число равных корзин, заданная ширина, ручные границы;
- гистограмма (ось X — границы сумм, ось Y — число строк) и таблица частот.

**Параметры по умолчанию:** `config.json` (также продублированы внутри HTML для автономной работы через `file://`).
