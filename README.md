...
└── POST/                         # Копии файлов кода с расширением .txt (для обмена/архива)
    ├── index.html.txt
    ├── button-types-preview.html.txt
    └── design-preview.html.txt
```
...
- В каталоге `POST` хранятся копии всех файлов кода с добавленным расширением `.txt` (для передачи или архивирования без потери содержимого)
...
5. **Копирование ссылки** — Ctrl+клик (или Cmd+клик на Mac) по карточке копирует её URL в буфер обмена без перехода по ссылке
6. **Сгенерированные скрипты** копируются в буфер обмена и запускаются в DevTools (F12) на соответствующих страницах
...
- **Копирование в буфер:** используется Clipboard API с запасным вариантом через `document.execCommand('copy')` (в т.ч. для SberBrowser).
