...
### Версия 2.3.0 (Текущая)
**Дата обновления:** 2025-06-10

**Основные изменения:**
- ✅ **Рефакторинг группы «Администрирование»** — оставлены декодер permission, проверка роли (ПСИ), экспорт ссылок; добавлены 8 кнопок копирования DevTools-скриптов (`scriptFile`)
...
- DevTools-скрипты редактируются в `JS/`; для встраивания в `index.html` (работа через `file://`): `python3 tools/embed_scripts.py`

## Использование

1. **Откройте `index.html`** в браузере двойным щелчком (`file://`) — HTTP-сервер не нужен
...
**Обновление скриптов после правок в `JS/`:**
```bash
python3 tools/embed_scripts.py   # встроить в index.html (для file://)
