...
├── sum-distribution.html         # Распределение сумм по ТН (гистограмма)
├── button-types-preview.html     # Превью типов кнопок
├── design-preview.html           # Превью дизайна
├── config.json                   # Параметры sum-distribution (дублируются в HTML)
├── ROADMAP.md                    # Roadmap страницы распределения сумм
├── README.md                     # Документация проекта
├── .gitignore                    # В т.ч. IN/, JS/IN/, log/, .env
├── samples/
│   └── sum-distribution-demo-1800.csv  # Демо-данные для гистограммы
├── tools/
...
- интервалы: равная ширина, заданная ширина, **ручные границы** — ползунки **под** гистограммой + **поля ввода** значений (синхрон с бегунками); график `position: sticky`;
- **легенда** с цветами сразу под canvas (не ниже таблицы);
- гистограмма (Canvas) + таблица частот;
...
**Параметры:** `config.json` (продублированы в HTML для `file://`). Описание roadmap: `ROADMAP.md`.
