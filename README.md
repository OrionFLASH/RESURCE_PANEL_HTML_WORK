...
**Возможности:**
- фильтры: **дерево ТБ→ГОСБ** (активные сверху, серые выбираемые; ГОСБ→ТБ, ТБ→все ГОСБ); кластеры отдельно;
- разрез: вся выборка / ТБ / ГОСБ / ТБ+ГОСБ / кластер;
- **группы на графике**: «Интервалы сумм» или «Значения разреза» (при выборе разреза ТБ/ГОСБ/… по умолчанию — **Значения**);
- интервалы: только **ручные границы** (ползунки под графиком) — **целые**; кнопки **«Равные интервалы»**, **«Равно по числу ТН»**, **«Лесенка»**; бейдж номера группы на бегунке; поля ввода; график `position: sticky`;
...
- **легенда** с цветами сразу под canvas (не ниже таблицы);
- гистограмма (Canvas) + подписи **числа ТН** на столбиках (−90° или над столбиком, цвет по серии); адаптивные подписи оси X;
- **hover по столбику** — подсказка со статистикой (интервал, серия, ТН/ТБ/ГОСБ/кластер);
- **детальная статистика по интервалам**: ТБ / ГОСБ / кластер / ТН, **% всех** и **% в ТБ/ГОСБ/кластере**, строка **Итого**; при разрезе — блоки по значениям;
...
- сетка: настройки слева, график справа (график шире).

**Демо-данные** (каталог `samples/` в `.gitignore`, в репозиторий не коммитить):

| Файл | Как получить |
|------|----------------|
| `samples/sum-distribution-demo-20000.csv` | `python3 tools/build_sum_demo_csv_20k.py` — 20 000 строк, 2 000 уник. ТН, повторы 1…50, разные суммы |
| `samples/sum-distribution-demo-1800.csv` | `python3 tools/build_sum_demo_csv.py` — из локальных `IN/` (если есть) |

Пересборка 20k (без `IN/`):
