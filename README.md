...
**Возможности:**
- фильтры по ТБ / ГОСБ / кластеру (если колонки есть) — **каскадные**: при выборе ТБ в ГОСБ/кластере доступны только связанные значения (остальные ниже, серые, disabled); пустой выбор по полю = без фильтра по нему; конфликты снимаются автоматически;
...
- **детальная статистика по интервалам**: колонки ТБ / ГОСБ / кластер / ТН (абсолюты), строка **Итого** внизу; при разрезе — отдельный блок на каждое значение;
