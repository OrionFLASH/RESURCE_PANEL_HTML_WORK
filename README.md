...
- ✅ **Демо-CSV** — `samples/sum-distribution-demo-1800.csv` (генератор `tools/build_sum_demo_csv.py` из локальных `IN/`)
...
│   └── build_sum_demo_csv.py     # Сборка демо-CSV из локальных IN/
...
**Возможности:**
- фильтры по ТБ / ГОСБ / кластеру (если колонки есть);
- разрез: вся выборка / ТБ / ГОСБ / ТБ+ГОСБ / кластер;
- интервалы: равная ширина, заданная ширина, **ручные границы** — поле «Число границ» = число ползунков; шкала `min — ●…● — max` (края фиксированы, порядок сохраняется);
- полупрозрачные подсказки при наведении;
- сетка: настройки слева, график справа (график шире).

**Демо-данные:** [`samples/sum-distribution-demo-1800.csv`](samples/sum-distribution-demo-1800.csv) (1800 уникальных ТН; суммы в рублях). Пересборка из локальных `IN/` (не в git):
