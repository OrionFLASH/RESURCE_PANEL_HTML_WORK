...
### Версия 2.4.4 (Текущая)
**Дата обновления:** 2026-08-10

**Основные изменения:**
- ✅ **Подробная статистика по интервалам** — ТБ / ГОСБ / кластер / ТН как «попало / всего»; при разрезе — отдельный блок на каждое значение
...
- **группы на графике**: «Интервалы сумм (внутри — разрез)» или «Значения разреза (внутри — распределение)»;
...
- гистограмма (Canvas) + **детальная статистика по интервалам**: ТБ / ГОСБ / кластер / ТН как «попало / всего в блоке»; при разрезе — отдельный блок на каждое значение (ТБ 1, ТБ 2…);
...
- полупрозрачные подсказки при наведении;
